...
export default function LoginPage() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (username === 'administrator' && password === '12345678') {
      router.push('/peminjaman'); // Arahkan ke rute /peminjaman
    } else {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen font-sans p-4">
      ...
      <form onSubmit={handleSubmit} className="bg-gray-300 p-10 rounded-lg shadow-xl w-full max-w-md">
        ...
        <button
          type="submit"
          className="bg-gray-700 hover:bg-blue-700 text-white font-bold py-3 px-4 rounded focus:outline-none focus:shadow-outline w-full transition duration-300 ease-in-out"
        >
          Login
        </button>
      </form>
